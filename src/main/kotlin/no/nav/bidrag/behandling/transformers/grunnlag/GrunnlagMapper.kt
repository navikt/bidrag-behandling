...
class GrunnlagMapper(
    ...
    fun byggGrunnlagForBeregning(
        ...
        mapper.run { behandling.tilPersonobjekter(søknadsbarnRolle) }
        val personobjekter = mapper.opprettPersonobjekter(behandling, søknadsbarnRolle)
        val søknadsbarn = mapper.rolleTilGrunnlagPerson(søknadsbarnRolle)
        val bostatusBarn = mapper.opprettGrunnlagBostatus(behandling, personobjekter)
        val inntekter = mapper.opprettGrunnlagInntekt(behandling, personobjekter, søknadsbarn, false)
        val grunnlagsliste = (personobjekter + bostatusBarn + inntekter).toMutableSet()

        when (behandling.tilType()) {
            TypeBehandling.FORSKUDD ->
                grunnlagsliste.addAll(
                    mapper.opprettGrunnlagSivilstand(
                        behandling,
                        personobjekter.bidragsmottaker ?: manglerRolleIGrunnlag(Rolletype.BIDRAGSMOTTAKER, behandling.id),
                    ),
                )

            TypeBehandling.SÆRBIDRAG -> {
                grunnlagsliste.add(mapper.opprettGrunnlagUtgift(behandling))
            }

            else -> {}
        }
        val beregnFraDato = behandling.virkningstidspunkt ?: vedtakmappingFeilet("Virkningstidspunkt må settes for beregning")
        val beregningTilDato = finnBeregnTilDato(behandling.virkningstidspunkt!!)
    ...
    fun byggGrunnlagForVedtak(
        behandling: Behandling,
        personobjekterFraBeregning: MutableSet<GrunnlagDto> = mutableSetOf(),
    ): Set<GrunnlagDto> {
        val personobjekter = (mapper.opprettPersonobjekter(behandling) + personobjekterFraBeregning).toSet()
        val bostatus = mapper.opprettGrunnlagBostatus(behandling, personobjekter)
        val personobjekterMedHusstandsmedlemmer =
            (personobjekter + bostatus.husstandsmedlemmer()).toMutableSet()
        val innhentetGrunnlagListe = mapper.opprettInnhentetGrunnlag(behandling, personobjekterMedHusstandsmedlemmer)
        val inntekter = mapper.opprettGrunnlagInntekt(behandling, personobjekter)
        ...
        when (behandling.tilType()) {
            ...
            TypeBehandling.SÆRBIDRAG ->
                grunnlagListe.addAll(
                    behandling.byggGrunnlagUtgiftsposter() + behandling.byggGrunnlagUtgiftDirekteBetalt() +
                        behandling.byggGrunnlagUtgiftMaksGodkjentBeløp(),
                )
            else -> {}
        }
