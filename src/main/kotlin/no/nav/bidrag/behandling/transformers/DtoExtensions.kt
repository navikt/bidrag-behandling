...
fun Behandling.tilForsendelseRolleDto() = roller.filter { r -> !(r.rolleType == Rolletype.BARN && r.ident == null) }.map {
    ForsendelseRolleDto(
...
fun CreateRolleDto.toRolle(behandling: Behandling): Rolle = Rolle(
    behandling,
    rolleType =
    when (this.rolleType) {
...
fun SoknadType.tilVedtakType(): Vedtakstype = when (this) {
